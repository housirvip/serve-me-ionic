...
export class ProfilePage implements OnInit {
  ...
  async updateEmailModal() {
    if (!this.user.phone) {
      this.toastService
        .presentToast(
          "if you want to change email please verify your phone number first! ",
          2000
        )
        .then(r => {});
      return;
  ...
  async updatePasswordModal() {
    if (!this.user.phone) {
      this.toastService
        .presentToast(
          "if you want to change password please verify your phone number first! ",
          2000
        )
        .then(r => {});
      return;
      ...
      buttons: [
        {
          ...
          handler: () => {
            this.imageService.getImageFromLibrary().then(imageData => {
              ...
              const storageRef = firebase
                .storage()
                .ref(
                  "static/photo/" + this.userService.user.firebaseUid + ".jpg"
                );
              this.presentLoading();
              storageRef
                ...
                .catch(err => {
                  this.toastService
                    .presentToast("upload failed! ", 2000)
                    .then(r => {});
                });
            });
          }
        },
        {
          text: "Use Camera",
          handler: () => {
            this.imageService.getImageFromCamera().then(imageData => {
              const base64Image = "data:image/jpeg;base64," + imageData;
              // const base64Image =  imageData;
              const storageRef = firebase
                .storage()
                .ref("static/photo" + this.userService.user.firebaseUid);
              this.presentLoading();
              storageRef
                .putString(base64Image, "data_url")
                .then(snapshot => {
